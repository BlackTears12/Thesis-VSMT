#include "resourcemonitor_qnx.hpp"
#include <QStorageInfo>

namespace metrics::monitor
{

ResourceMonitor<Platform::QNX>::CpuMonitor::CpuMonitor()
{
...
CpuInfo ResourceMonitor<Platform::QNX>::CpuMonitor::gatherProcessorInfo()
{
    CpuInfo info;
    /*try {
...
vector<double> ResourceMonitor<Platform::QNX>::CpuMonitor::gatherCoreLoads()
{
    vector<double> loads;
...
double ResourceMonitor<Platform::QNX>::CpuMonitor::calculateLoadForCore(const vector<unsigned int> &data, unsigned int coreIdx)
{
    // idle = idle + iowait
...
vector<NetworkInterfaceInfo> ResourceMonitor<Platform::QNX>::NetworkMonitor::gatherInfo()
{
    updateCurrentLoads();
...
NetworkInterfaceInfo ResourceMonitor<Platform::QNX>::
...
void ResourceMonitor<Platform::QNX>::NetworkMonitor::updateCurrentLoads()
{
...
unique_ptr<SystemInfo> ResourceMonitor<Platform::QNX>::gatherSysInfo()
{
    if(!sysInfoCache) {
...
MemoryInfo ResourceMonitor<Platform::QNX>::gatherMemoryInfo() const
{
    MemoryInfo info;
    /*try {
...
ProcessInfo ResourceMonitor<Platform::QNX>::parsePSCommandRow(const QString &line) const
{
...
vector<ProcessInfo> ResourceMonitor<Platform::QNX>::gatherProcessInfo() const
{
...
unique_ptr<RuntimeMetric> ResourceMonitor<Platform::QNX>::gatherRuntimeMetric()
{
...
vector<StorageInfo> ResourceMonitor<Platform::QNX>::gatherStorageInfo() const
{
...
vector<CanBusDeviceInfo> ResourceMonitor<Platform::QNX>::gatherCanDevInfo() const
{
    return {};
}
